...
- [ ] Exact tested Kubernetes and Gateway API versions, OpenStack and Octavia
      ...
      networks are recorded below. Missing OpenStack test or conformance
...
- [ ] GitHub private vulnerability reporting is enabled before the first
      public release.
- [ ] An annotated or signed SemVer tag with a `v` prefix was created at the
      reviewed commit and pushed without moving or reusing an existing tag.
- [ ] The Release workflow succeeded and created a draft GitHub release.
- [ ] The draft contains one binary archive for each supported architecture,
      one source archive, and `checksums.txt`, with names and versions matching
      the tag.
- [ ] The pre-alpha warning, reviewed Changelog block, compatibility evidence,
      and known limitations were copied and reviewed. Notes generated by the
      workflow were treated only as a starting point.
